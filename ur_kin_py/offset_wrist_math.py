...
Copy pasted from: https://github.com/compas-dev/compas_fab/blob/main/src/compas_fab/backends/kinematics/solvers/offset_wrist.py

Instead of importing compas, certain functions have been reimplemented here.

from compas.geometry import Frame -> Use numpy array
from compas_fab.utilities import sign -> See function below
...
IK_SOLUTION_DESCRIPTION = {
    ...
    2: {"Shoulder": "Left",  "Wrist": "Down", "Elbow": "Up"},
    3: {"Shoulder": "Left",  "Wrist": "Down", "Elbow": "Down"},
    4: {"Shoulder": "Right", "Wrist": "Up",   "Elbow": "Up"},
    5: {"Shoulder": "Right", "Wrist": "Up",   "Elbow": "Down"},
    6: {"Shoulder": "Right", "Wrist": "Down", "Elbow": "Up"},
    7: {"Shoulder": "Right", "Wrist": "Down", "Elbow": "Down"},
}

# from compas.geometry import Frame
# from compas_fab.utilities import sign
# Taken from https://github.com/compas-dev/compas_fab/blob/main/src/compas_fab/utilities/numbers.py
def sign(number): 
...
def inverse_kinematics_offset_wrist(frame, params, q6_des=0.0) -> list[list[float]]:
    ...
    solutions = []
    # seems like you flip X and Y....
    # T02, T12, T22 = frame.xaxis
    # T00, T10, T20 = frame.yaxis
    # T01, T11, T21 = frame.zaxis
    # T03, T13, T23 = frame.point
    T02, T12, T22 = frame[0:3, 0]
    T00, T10, T20 = frame[0:3, 1]
    T01, T11, T21 = frame[0:3, 2]
    T03, T13, T23 = frame[0:3, 3]
    T02 *= -1
    T03 *= -1
    T12 *= -1
    T13 *= -1
    ...
    if fabs(A) < ZERO_THRESH:
        ...
        if abs(div) > 1.0: # added this bc sometime div > 1, which causes asin to fail
            return [None]*8
...
if __name__ == "__main__":
    ...
    q = [0.2, 5.5, 1.4, 1.3, 2.6, 3.6] # goes outside [-pi,pi], but no wrapping so its ok
    frame = forward_kinematics_offset_wrist(q, params)
    sol = inverse_kinematics_offset_wrist(frame, params)
    print(f"Num Solutions: {len(sol)}")
